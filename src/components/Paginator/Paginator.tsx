...
export const Paginator: React.FC<PaginatorPropsType> = ({ currentPage, onPageChanged, totalPagesCount }) => {
    ...
    if (currentPage + 5 >= totalPagesCount) {
        for (let i = totalPagesCount - 6; i <= totalPagesCount; i++) {
            pages.push(i)
        }
    }
    ...
    return (
        <div className="flex text-gray items-center justify-around mt-12">
            ...
            {pages.map(page => <div key={page} onClick={() => onPageChanged(page)} className={`w-12 h-12 border-2 cursor-pointer rounded-full flex items-center justify-center ${page === currentPage ? 'border-yellow-400' : ''} hover:border-yellow-400 transition-all`}>{page}</div>)}
